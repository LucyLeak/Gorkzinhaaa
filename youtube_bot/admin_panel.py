...
from youtube_bot.youtube.client import YouTubeQuotaExceededError
...
:root{--bg:#080c16;--surface:#111827;--surface2:#17233a;--line:#293956;--text:#e9efff;--muted:#9aa9c5;--accent:#76a9ff;--ok:#38d39f;--danger:#ff6b7a;--warn:#f7c96b;--shadow:0 16px 50px #0005} [data-theme=light]{--bg:#f3f6fc;--surface:#fff;--surface2:#edf3ff;--line:#d5deee;--text:#17223a;--muted:#5c6b85;--accent:#356ee8;--shadow:0 10px 30px #34507822}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px Inter,system-ui,sans-serif}button,input,select,textarea{font:inherit}button{cursor:pointer}.shell{display:grid;grid-template-columns:250px 1fr;min-height:100vh}.side{background:var(--surface);border-right:1px solid var(--line);padding:22px 14px;position:sticky;top:0;height:100vh}.brand{font-size:20px;font-weight:800;color:var(--accent);padding:0 12px 26px}.nav{display:grid;gap:5px}.nav button{border:0;background:transparent;color:var(--muted);text-align:left;padding:12px;border-radius:10px}.nav button:hover,.nav button.active{background:var(--surface2);color:var(--text)}.side-foot{position:absolute;bottom:20px;left:25px;color:var(--muted);font-size:12px}.side-foot a{color:var(--accent)}main{min-width:0}.top{height:70px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;padding:0 28px;background:var(--surface)}.top h1{font-size:18px;margin:0}.status{color:var(--muted)}.dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--ok);margin-right:7px}.content{max-width:1200px;padding:28px;margin:auto}.panel{display:none}.panel.active{display:block}.card{background:var(--surface);border:1px solid var(--line);border-radius:14px;padding:20px;margin-bottom:18px;box-shadow:var(--shadow)}h2{font-size:16px;margin:0 0 8px}.muted{color:var(--muted);font-size:13px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.stat{background:var(--surface2);border-radius:11px;padding:15px}.stat b{display:block;font-size:23px;color:var(--accent)}label{display:block;color:var(--muted);font-size:12px;margin:12px 0 5px}input,select,textarea{width:100%;background:var(--bg);color:var(--text);border:1px solid var(--line);border-radius:8px;padding:10px}textarea{min-height:100px;resize:vertical}.row{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.btn{border:1px solid var(--line);background:var(--surface2);color:var(--text);padding:9px 14px;border-radius:8px}.btn.primary{background:var(--accent);color:#fff;border-color:var(--accent)}.btn.danger{color:#fff;background:var(--danger);border-color:var(--danger)}.btn.small{padding:5px 9px;font-size:12px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:11px 9px;border-bottom:1px solid var(--line);white-space:nowrap}th{color:var(--muted);font-size:12px}.badge{border-radius:99px;padding:3px 9px;font-size:11px;background:var(--surface2)}.badge.ok{color:var(--ok)}.badge.bad{color:var(--danger)}.badge.wait{color:var(--warn)}pre{background:var(--bg);padding:14px;overflow:auto;border-radius:8px;min-height:80px}.toast{position:fixed;right:22px;bottom:22px;background:var(--surface2);border:1px solid var(--line);padding:12px 16px;border-radius:9px;box-shadow:var(--shadow);z-index:4}.mobile{display:none}@media(max-width:760px){.shell{display:block}.side{height:auto;position:static;border-right:0;border-bottom:1px solid var(--line);padding:12px}.brand{padding:8px}.nav{display:flex;overflow:auto}.nav button{white-space:nowrap}.side-foot{display:none}.top{padding:0 16px}.content{padding:16px}.mobile{display:block}}
 </style></head><body><div class="shell"><aside class="side"><div class="brand">? Gorkzinhaaa</div><nav class="nav" aria-label="Navegação"> <button class="active" data-panel="tts">??? Fila TTS</button><button data-panel="live">🔴 Live</button><button data-panel="tts-test">?? Teste TTS</button><button data-panel="elevenlabs">?? ElevenLabs</button><button data-panel="personalidades">?? Personalidades</button><button data-panel="cleanup">?? Limpeza</button><button data-panel="terminal">? Terminal</button><button data-panel="clients">?? API Clients</button><button data-panel="quota">Quota</button></nav><div class="side-foot"><a href="/docs">Documentação da API</a><br><a href="/">Início</a></div></aside><main><header class="top"><h1 id="title">Fila de aprovação</h1><div class="row"><button class="btn small" id="theme" aria-label="Alternar tema">? Tema</button><span class="status"><i class="dot" id="dot"></i><span id="statusText">Conectando</span></span></div></header><section class="content">
<section class="panel active" id="panel-tts"><div class="card"><h2>Fila de aprovação TTS</h2><p class="muted">Aprove ou rejeite os áudios produzidos pelo bot. TTS antigos são removidos automaticamente após o período configurado de retenção.</p><label><input type="checkbox" id="hideAdminTests" style="width:auto"> Ocultar testes administrativos</label><div class="table-wrap" id="ttsQueue"></div></div></section>
<section class="panel" id="panel-live"><div class="card"><h2>Controle da live</h2><div id="liveStatus" class="muted">Carregando status...</div><div class="row"><button class="btn primary" onclick="forceLiveCheck()">Forçar conexão com live</button><button class="btn danger" onclick="disconnectLive()">Desconectar live</button></div><pre id="liveActionOutput"></pre></div></section>
<section class="panel" id="panel-quota"><div class="card"><h2>Cota do YouTube</h2><p class="muted">Atualiza��o autom�tica a cada 30 segundos.</p><div id="quotaCard">Carregando...</div></div></section><section class="panel" id="panel-tts-test"><div class="card"><h2>Terminal de teste TTS</h2><p class="muted">Testes ficam no histórico e não interferem no processamento.</p><div class="grid"><div><label>Provedor</label><select id="ttsTestProvider"><option>gtts</option><option>edge</option><option>openai</option><option>elevenlabs</option></select></div><div><label>Voz / idioma</label><input id="ttsTestVoice" value="pt"></div></div><label>Texto</label><textarea id="ttsTestText" maxlength="300" placeholder="Digite o texto para sintetizar..."></textarea><button class="btn primary" onclick="runTtsTest()">Gerar Áudio</button><pre id="ttsTestOutput"></pre><audio id="ttsTestAudio" controls style="display:none;width:100%"></audio></div></section>
...
function loadLiveStatus(){api('/live-status').then(r=>{liveStatus.textContent=r.connected?`Conectado: ${r.video_id||'-'} | ${r.title||'-'} | desde ${r.connected_since||'-'} | mensagens: ${r.messages_processed||0}`:(r.outside_window?'Fora da janela. Próxima checagem: '+(r.next_scheduled_check||'-'):'Nenhuma live conectada.')})}function loadQuotaStatus(){api('/quota-status').then(r=>{if(r.error){quotaCard.textContent='Erro: '+r.error;return}let pct=r.limit?Math.max(0,Math.min(100,(r.remaining/r.limit)*100)):0;let color=pct>50?'#35c978':pct>=20?'#e6b84a':'#ef596f';quotaCard.innerHTML=`<div>Usado: <b>${r.used||0}</b> / ${r.limit||0} unidades</div><div>Restante: <b>${r.remaining||0}</b></div><div style="height:10px;background:#293956;border-radius:8px;margin:10px 0"><div style="height:100%;width:${pct}%;background:${color};border-radius:8px"></div></div><div>Próximo reset BRT: ${r.reset_brt||'-'}</div><div>Próximo reset PT: ${r.reset_pt||'-'}</div>`})}setInterval(loadQuotaStatus,30000);function loadAudioStats(){api('/audio-stats').then(r=>audioStats.innerHTML=`<div class=stat><b>${r.total_files||0}</b>arquivos</div><div class=stat><b>${r.total_mb||0} MB</b>total</div><div class=stat><b>${r.oldest_file||'-'}</b>mais antigo</div>`)}function runCleanup(e){cleanupResult.textContent='Executando?';api('/cleanup',{method:'POST',body:JSON.stringify({execute:e})}).then(r=>{cleanupResult.textContent=JSON.stringify(r,null,2);loadAudioStats();toast('Concluído')})}function runSQL(){api('/terminal',{method:'POST',body:JSON.stringify({sql:sqlInput.value})}).then(r=>sqlResult.textContent=JSON.stringify(r,null,2))}
...
class AdminPanel:
    ...
    def __init__(
        ...
        on_force_live_check: Callable[[], Awaitable[dict]] | None = None,
        on_disconnect_live: Callable[[], Awaitable[dict]] | None = None,
        get_live_status: Callable[[], dict] | None = None,
        ...
        self.on_force_live_check = on_force_live_check
        self.on_disconnect_live = on_disconnect_live
        self.get_live_status = get_live_status
    ...
    def set_live_controls(
        ...
        self.on_force_live_check = force_check
        self.on_disconnect_live = disconnect
        self.get_live_status = status

    async def handle_force_live_check(self, request: web.Request) -> web.Response:
        if not self._check_auth(request):
            return self._auth_error()
        if self.on_force_live_check is None:
            return web.json_response({"ok": False, "error": "live_control_unavailable"}, status=503)
        try:
            return web.json_response(await self.on_force_live_check())
        except (QuotaGuardTriggered, YouTubeQuotaExceededError, RuntimeError) as exc:
            if isinstance(exc, (QuotaGuardTriggered, YouTubeQuotaExceededError)) or "quota" in str(exc).lower():
                return web.json_response(quota_exceeded_payload(), status=429)
            logger.exception("[MANUAL] Force live check failed.")
            return web.json_response({"ok": False, "error": "internal_error"}, status=500)
        except Exception:
            logger.exception("[MANUAL] Force live check failed.")
            return web.json_response({"ok": False, "error": "internal_error"}, status=500)

    async def handle_disconnect_live(self, request: web.Request) -> web.Response:
        if not self._check_auth(request):
            return self._auth_error()
        if self.on_disconnect_live is None:
            return web.json_response({"ok": False, "error": "live_control_unavailable"}, status=503)
        try:
            return web.json_response(await self.on_disconnect_live())
        except (QuotaGuardTriggered, YouTubeQuotaExceededError, RuntimeError) as exc:
            if isinstance(exc, (QuotaGuardTriggered, YouTubeQuotaExceededError)) or "quota" in str(exc).lower():
                return web.json_response(quota_exceeded_payload(), status=429)
            logger.exception("Manual live disconnect failed.")
            return web.json_response({"ok": False, "error": "internal_error"}, status=500)
        except Exception:
            logger.exception("Manual live disconnect failed.")
            return web.json_response({"ok": False, "error": "internal_error"}, status=500)

    async def handle_live_status(self, request: web.Request) -> web.Response:
        if not self._check_auth(request):
            return self._auth_error()
        result = self.get_live_status() if self.get_live_status else {"connected": False}
        if self.quota_tracker:
            quota = await self.quota_tracker.status()
            if quota["remaining"] <= 0:
                return web.json_response(quota_exceeded_payload(), status=429)
            result["quota"] = quota
        return web.json_response(result)

    async def handle_quota_status(self, request: web.Request) -> web.Response:
        if not self._check_auth(request):
            return self._auth_error()
        if not self.quota_tracker:
            return web.json_response({"error": "quota_tracker_unavailable"}, status=503)
        return web.json_response(await self.quota_tracker.status())
    ...
    def register_routes(self, app: web.Application) -> None:
        ...
        app.router.add_post("/admin/api/force-live-check", self.handle_force_live_check)
        app.router.add_post("/admin/api/disconnect-live", self.handle_disconnect_live)
        app.router.add_get("/admin/api/live-status", self.handle_live_status)
        app.router.add_get("/admin/api/quota-status", self.handle_quota_status)
