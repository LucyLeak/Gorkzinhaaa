...
function loadLiveStatus(){api('/live-status').then(r=>{liveStatus.textContent=r.connected?`Conectado: ${r.video_id||'-'} | ${r.title||'-'} | desde ${r.connected_since||'-'} | mensagens: ${r.messages_processed||0}`:(r.outside_window?'Fora da janela. Próxima checagem: '+(r.next_scheduled_check||'-'):'Nenhuma live conectada.')})}function loadAudioStats(){api('/audio-stats').then(r=>audioStats.innerHTML=`<div class=stat><b>${r.total_files||0}</b>arquivos</div><div class=stat><b>${r.total_mb||0} MB</b>total</div><div class=stat><b>${r.oldest_file||'-'}</b>mais antigo</div>`)}function runCleanup(e){cleanupResult.textContent='Executando?';api('/cleanup',{method:'POST',body:JSON.stringify({execute:e})}).then(r=>{cleanupResult.textContent=JSON.stringify(r,null,2);loadAudioStats();toast('Concluído')})}function runSQL(){api('/terminal',{method:'POST',body:JSON.stringify({sql:sqlInput.value})}).then(r=>sqlResult.textContent=JSON.stringify(r,null,2))}
...
class AdminPanel:
    ...
    async def handle_live_status(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            self.get_live_status() if self.get_live_status else {"connected": False}
        )
